...
`ThemePanel` also gains a `customColors` prop (default `true`): the bookmarklet turns it off, since custom colors map to no shadcn variable and would be a control that does nothing there.
