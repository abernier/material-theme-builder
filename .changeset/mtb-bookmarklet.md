...
`ThemePanel` also gains two props: `customColors` (default `true`), which the bookmarklet turns off since custom colors map to no shadcn variable and would be a control that does nothing there, and `size` (`"default" | "lg"`), which scales every control up for an overlay meant to be hit on any page.
